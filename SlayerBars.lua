...
local MAIN_STATUS = SlayerBarStatus
...
function StackedBar:Initialize(unitTag, existingControl)
	...
	self.leadshineIdleAnim = ANIMATION_MANAGER:CreateTimelineFromVirtual("SlayerBarsLeadingShineIdleAnim", self.leadshine)
	...
	self.leadshineIdleAnim:PlayFromStart()
...
function StackedBar:CombatOutgoing(result, isError, abilityName, abilityGraphic, abilityActionSlotType, sourceName, sourceType, targetName, targetType, hitValue, powerType, damageType, log, sourceUnitId, targetUnitId, abilityId, overflow)
	local test = debounce( function() self.leadshineHitAnim:PlayFromStart() end, 800 )
...
local function OnLoaded(_, name)
	...
	local MAIN_BAR = StackedBar:New("reticleover", SlayerBarStatus)
